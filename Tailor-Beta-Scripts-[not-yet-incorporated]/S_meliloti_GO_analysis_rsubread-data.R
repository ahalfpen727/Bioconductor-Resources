...
biocLite("GOstats");biocLite("topGO");library(topGO)
biocLite(c("KEGGgraph","KEGG.db","gage","pathview",
           "clusterProfiler","STRINGdb","clusterProfiler"))
biocLite(c("keggorthology","EnrichmentBrowser"))
...
library(keggorthology);library(Rsubread)
...
###########################################################################################
## Query STRINGdb database for Hsapiens
###########################################################################################
...
string.db.hsa
hsa.kegg<-search_kegg_organism('hsa', by='kegg_code')
hsa.kegg.org<- search_kegg_organism('Homo sapiens', by='scientific_name')
dim(hsa.kegg.org)
head(hsa.kegg.org)
...
head(hsa.pwys)
head(kegg.gs)
keggfile<-file.path("Hsa.kegg.genesets.txt", "w")
keggfile<-file("Hsa.kegg.genesets.txt", "a")
KEGGid<-names(kegg.gs)
x=0
for (keggpath in kegg.gs){
  x<-c(x + 1)
