...
library(cummeRbund);library(igraph);library(limma)
library(pvclust); library(gplots)
...
cor(s.o.g.rep.ma)
s.o.g.rep.df<-as.data.frame(t(over.grp.s.o.g.rep.ma))
sig.gse.cor<-cor(s.o.g.rep.df)
#plotcorr(sig.gse.cor,mar = c(0.1, 0.1, 0.1, 0.1))
# Do the same, but with colors corresponding to value
colorfun <- colorRamp(c("#CC0000","white","#3366CC"), space="Lab")
...
dens2<-csDensity(genes(cuff),replicates=T,logMode=T,pseudocount=1)
print(dens2)
dens3<-csDensity(isoforms(cuff), replicates=T,logMode=T,pseudocount=1)
print(dens3)
