...
#library(GEOquery)
library(limma);library(readxl);library(edgeR)
#@ evolutionary oonservation
# flybase
# whittle biomarker set down to roughly 30 features
#######################################################################
### code chunk number 2: Load the count matrix
#######################################################################
setwd("../../../../umb_triley/drosophila")
setwd("umb_triley/drosophila")
files<-dir()
files
...
sig.g.rep.df<-as.data.frame(sig.g.rep.matrix)
sig.gse.cor<-cor(sig.g.rep.df)
