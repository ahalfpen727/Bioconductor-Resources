...
while IFS='' read -r line || [[ -n "$line" ]]; do   # each line is a directory that contains a accepted_hit.bam file
    ...
    if [[ $line =~ $SAMTOOLS_INPUT_REGEX ]]; then
        ...
        COMMAND_LINE="${COMMAND} stats ${INPUT}/${SAMPLE}_out/accepted_hits.bam > ${OUTPUT}/${SAMPLE}_out.stats; ${COMMAND} stats ${INPUT}/${SAMPLE}_out/unmapped.bam > ${OUTPUT}/${SAMPLE}_out.unmapped.stats"
