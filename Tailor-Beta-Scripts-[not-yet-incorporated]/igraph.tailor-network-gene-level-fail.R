...
library(ReactomePA);library(GSEABase)
library(GO.db); library(org.Hs.eg.db)
library(KEGG.db);library(reactome.db);
library(gage);library(pathview)
library(limma);library(ReactomePA)
library(KEGGgraph);library(keggorthology)
...
cuff.hg38<-cuff
diffDir<-file.path("~/umb_triley/urine1/cuffdiff_results_hg19_default/LUTS-over-CTRL")
...
length(mySigGeneTable); head(mySigGeneTable,20)
...
head(sig_genes_exp.diff);dim(sig_genes_exp.diff)
head(sig_genes_annot);dim(sig_genes_annot)
sig_gene.df  <- subset(gene_exp.diff, (log2_fold_change > 5 & q_value <= 0.01))
head(sig_gene_data);dim(sig_gene_data)
sig_gene_ids <- c(sig_gene_data$gene_id)

g.rep.matrix<-repFpkmMatrix(genes(cuff))
head(g.rep.matrix)

s.g.rep.matrix<-subset(s.g.rep.matrix, row.names(s.g.rep.matrix) %in% sig_gene_ids)
...
#gene.list<-getGenes(cuff,geneId = gene.xloc.matrix); gene.list
#gene_annotation_data<-featureNames(gene.list@isoforms); head(gene_annotation_data)
...
#head(gene_annotation_data); dim(gene_annotation_data)
#gene.exp.diff<-cbind(gene_annotation_data, gene_exp.diff)
#gene_exp.diff["isoform_id"]<-gene_annotation_data["gene_short_name"];head(gene.exp.diff)
