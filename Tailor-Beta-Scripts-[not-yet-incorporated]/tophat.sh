...
PROCESSORS=5
MEMORY="5192"       # PER PROCESSOR!! - 2048=2G, 4096=4G, 8192=8G, 16384=16G, 32768=32G, 65536=64G
DURATION="60:00"   # HH:MM - 72:00=3 days, 96:00=4 days, 192:00=8 days, 384:00=16 days
...
while IFS='' read -r line || [[ -n "$line" ]]; do   # each line is a fastq file
	...
	scriptString="mktemp -p ${SCRIPTS} -t ${COMMAND}.${SAMPLE}.XXXXXXXXXXX"
	echo -e "\n${scriptString}"
...
#	COMMAND_LINE="${COMMAND} -p ${PROCESSORS} ${EXTRA_TOPHAT_PARAMETERS} -o ${OUTPUT}/${SAMPLE}_out ${BOWTIE_INDEX} ${INPUT}/${SAMPLE}_R1_val_1.fq.gz ${INPUT}/${SAMPLE}_R2_val_2.fq.gz"

COMMAND_LINE="${COMMAND} -p ${PROCESSORS} ${EXTRA_PARAMETERS} -o ${OUTPUT}/${SAMPLE}_out ${BOWTIE_INDEX} ${INPUT}/${SAMPLE}_R1_val_1.fq.gz ${INPUT}/${SAMPLE}_R2_val_2.fq.gz"

# The following modification was performed to analyze toxoplasmo data with tailor from an intermediate step
#COMMAND_LINE="${COMMAND} -p ${PROCESSORS} ${EXTRA_PARAMETERS} -o ${OUTPUT}/${SAMPLE}_out ${BOWTIE_INDEX} ${INPUT}/${SAMPLE}_R1.fastq ${INPUT}/${SAMPLE}_R2.fastq"
