...
fpkm_matrix=data.frame(gene.ids, LUTS_1,LUTS_2,LUTS_3,LUTS_4,LUTS_5,LUTS_6,LUTS_7,LUTS_8,CTRL_1,CTRL_2,CTRL_3,CTRL_4,CTRL_5,CTRL_6,CTRL_7,CTRL_8)
row.names(fpkm_matrix)=gene.ids 
data_columns=c(2:17)
short_names=c("LUTS_1","LUTS_2","LUTS_3","LUTS_4","LUTS_5","LUTS_6","LUTS_7","LUTS_8","CTRL_1","CTRL_2","CTRL_3","CTRL_4","CTRL_5","CTRL_6","CTRL_7","CTRL_8")

#Add ids as row names and gene names as initial column along with all data
FPKM_matrix=data.frame(LUTS_1,LUTS_2,LUTS_3,LUTS_4,LUTS_5,LUTS_6,LUTS_7,LUTS_8,CTRL_1,CTRL_2,CTRL_3,CTRL_4,CTRL_5,CTRL_6,CTRL_7,CTRL_8)
row.names(FPKM_matrix)=gene.ids 
data_columns=c(2:17)
short_names=c("LUTS_1","LUTS_2","LUTS_3","LUTS_4","LUTS_5","LUTS_6","LUTS_7","LUTS_8","CTRL_1","CTRL_2","CTRL_3","CTRL_4","CTRL_5","CTRL_6","CTRL_7","CTRL_8")
head(FPKM_matrix)
head(row.names(FPKM_matrix))
length(row.names(FPKM_matrix))
LUTSgroup<-as.data.frame(FPKM_matrix[,c("LUTS_1","LUTS_2","LUTS_3","LUTS_4","LUTS_5","LUTS_6","LUTS_7","LUTS_8")])
CTRLgroup<-as.data.frame(FPKM_matrix[,c("CTRL_1","CTRL_2","CTRL_3","CTRL_4","CTRL_5","CTRL_6","CTRL_7","CTRL_8")])
...
i = which(fpkm_matrix[,"gene_names"] == "CXCL12")
fpkm_matrix[i,]
...
status_counts1=table(tng_de[,"status"])
status_counts1
...
barplot(status_counts1, col=rainbow(6), xlab="Status", ylab="Transcript count", main="Gene status counts reported by Cuffdiff")

Status1=factor(tng_de[,"status"])
qplot(Status1, data=tng_de, geom="bar", fill=Status1, xlab="Status1", ylab="Transcript count", main="Status counts reported by Cuffdiff")


status_counts2=table(tnc_de[,"status"])
status_counts2
#Plot #2 - Now the same idea using ggplot2
Status2=factor(tnc_de[,"status"])
qplot(Status2, data=tnc_de, geom="bar", fill=Status, xlab="Status", ylab="Transcript count", main="Status counts reported by Cuffdiff")

status_counts3=table(tng_de[,"status"])
status_counts3
Status3=factor(tng_de[,"status"])
qplot(Status3, data=tng_de, geom="bar", fill=Status3, xlab="Status", ylab="Transcript count", main="Status counts reported by Cuffdiff")
...
xvf<-pie(status_counts1, col=rainbow(6), main="Status counts reported by Cuffdiff")
print(xvf)
#Plot #4 - Now the same idea using ggplot2
zz2=as.data.frame(status_counts1)
names(zz2) = c("Status", "Count")
pp1 <- ggplot(zz2, aes(x="", y=Count, fill=Status1)) + geom_bar(width=1) + coord_polar("y")
print(pp1)
### NOTE: The above needs to be updated as ggplot has changed###

#Plot #5 - Make a dotchart of these status counts
dotchart(as.numeric(status_counts1), col=rainbow(6), labels=names(status_counts1), xlab="Transcript count", main="Status counts reported by Cuffdiff", pch=16)
...
c_more_than_one = length(which(counts > 1))
c_max = max(counts)
hist(counts, breaks=5, col="bisque4", xlab="Transcripts per gene", main="Distribution of transcript count per gene")
plot(counts, breaks=5, col="bisque4", xlab="Transcripts per gene", main="Distribution of transcript count per gene")
legend_text = c(paste("Genes with one transcript =", c_one), paste("Genes with more than one transcript =", c_more_than_one), paste("Max transcripts for single gene = ", c_max))
legend("topright", legend_text, lty=NULL)
...
hist(tnc_fpkm[,"length"], breaks=50, xlab="Transcript length (bp)", main="Distribution of transcript lengths", col="steelblue")

hist(tng_fpkm[,"length"], breaks=50, xlab="Transcript length (bp)", main="Distribution of transcript lengths", col="steelblue")
...
boxplot(log2(fpkm_matrix[,data_columns]+min_nonzero), col=data_colors, names=short_names, las=2, ylab="log2(FPKM)", main="Distribution of FPKMs for all libraries")
...
x = fpkm_matrix[,"LUTS_1"]
y = fpkm_matrix[,"LUTS_2"]
...
x = fpkm_matrix[,"LUTS_2"]
y = fpkm_matrix[,"LUTS_3"]
plot(x=log2(x+min_nonzero), y=log2(y+min_nonzero), pch=16, col="blue", cex=0.25, xlab="FPKM (LUTS, Replicate 1)", ylab="FPKM (LUTS, Replicate 2)", main="Comparison of expression values LUTs_1 and LUTS_2")
#Add a straight line of slope 1, and intercept 0
abline(a=0,b=1)
#Calculate the correlation coefficient and display in a legend
...
plotCor2 = function(lib1, lib2, name, color){
  ...
  y=fpkm_matrix[,lib2]
  zero_count = length(which(x==0)) + length(which(y==0))
  counts = (length(x) + length(y))/2
  colors = colorRampPalette(c("white", "blue", "#007FFF", "cyan","#7FFF7F", "yellow", "#FF7F00", "red", "#7F0000"))
  smoothScatter(x=log2(x+min_nonzero), y=log2(y+min_nonzero), xlab=lib1, ylab=lib2, main=name, colramp=colors, nbin=275)
  abline(a=0,b=1)
  rs=cor(x,y, method="pearson")^2
  legend_text = c(paste("R squared = ", round(rs, digits=3), sep=""), paste("Zero count = ", zero_count, sep=""), paste("Counts = ", counts, sep=""))
  legend("topleft", legend_text, lwd=c(1,NA), col="black", bg="white", cex=0.8)
}
...
#Calculate the FPKM sum for all libraries
FPKM_matrix[,"sum"]=apply(FPKM_matrix[,data_columns2], 1, sum)
#Identify the genes with a grand sum FPKM of at least 5 - we will filter out the genes with very low expression across the board
i = which(FPKM_matrix[,"sum"] > 5)
#Calculate the correlation between all pairs of data
r=cor(FPKM_matrix[i,data_columns2], use="pairwise.complete.obs", method="pearson")
#Print out these correlation values
r
