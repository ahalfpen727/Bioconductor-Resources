...
bay.dat@libsizes <- getLibsizes(bay.dat)
...
ranking[1:nkeep,i] <- tc$status.keep
...
cat("TSPM\n")
i <- 11
tspm <- TSPM(counts=counts2, x1=as.factor(rep(c("A", "B"), each=3)), x0=rep(1,6), ib.size=colSums(counts2))
