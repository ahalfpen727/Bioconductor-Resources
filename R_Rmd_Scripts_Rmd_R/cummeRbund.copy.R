...
source("https://bioconductor.org/biocLite.R")
biocLite("cummeRbund", "gage", "gageData")
library(cummeRbund)
...
######################################################
print(args)
geneListString = Sys.getenv("GENE_LIST")
geneListString
gen_v = Sys.getenv("R_GENOME")
gen_v
#cuff<-readCufflinks(dir=inDir,gtfFile=refgtf,genome=gen_v,out=outDir,rebuild=T)
#cuff
...
head(hg38.genes)
hg38.ucsc_genes<-queryMany(transcript.rnas, scopes = "rna",fields="symbol",
                           species="human")
hg38.ucsc_genes<-queryMany(transcript.rnas, scopes = "rna",fields=c("symbol","entrezgene",,
                                                                    species="human")
...
dir()
dir("../../../RefGenomes/UCSC_hg38")
#cuff=readCufflinks(dir=".", gtfFile=file.path("../../cuffcompare_results_hg38_gtf_guided/cuffcmp.combined.gtf"),
#cuff=readCufflinks(dir=".", gtfFile=ucsc_hg38.gtf,runInfoFile = "run.info", repTableFile = "read_groups.info",geneFPKM = "genes.fpkm_tracking",
#geneDiff = "gene_exp.diff", geneCount="genes.count_tracking", geneRep="genes.read_group_tracking",
...
#CDSFPKM = "cds.fpkm_tracking", CDSExpDiff = "cds_exp.diff", CDSCount="cds.count_tracking",
#CDSRep="cds.read_group_tracking", CDSDiff = "cds.diff",promoterFile = "promoters.diff",
#splicingFile = "splicing.diff", varModelFile = "var_model.info", 
cuff=readCufflinks()
cuff
...
sig_gene_data  <- subset(gene_diff_data, (log2_fold_change > 1.5))
head(sig_gene_data)
sig_geneids <- c(sig_gene_data$gene_id)

myGenes <- getGenes(cuff, sig_geneids)
ep<-expressionBarplot(myGenes)
ep
...
head(g.rep.matrix)
tracking_id<-rownames(g.rep.matrix)
#g.rep.matrix<-as.data.frame(g.rep.matrix,row.names = tracking_id)
rownames(g.rep.matrix)
...
gene_annotation_data<-featureNames(gene.list)
head(gene_annotation_data)
tracking_id<-rownames(gene_annotation_data)
gene_id<-gene_annotation_data$gene_short_name
row.names(gene_annotation_data)<-tracking_id

gene_rep_matrix<-cbind(gene_annotation_data, g.rep.matrix)
head(gene_rep_matrix)
gene_rep_matrix<-gene_rep_matrix[,-1]
row.names(gene_rep_matrix)<-gene_rep_matrix$gene_short_name
gene_rep_matrix <- gene_rep_matrix[which(!duplicated(rownames(gene_rep_matrix))),]
head(gene_rep_matrix)
row.names(gene_rep_matrix)<-
dim(gene_rep_matrix)
colnames(gene_rep_matrix)

gene.rep.matrix<-gene_rep_matrix[,-1]
rownames(gene.rep.matrix)<-gene.rep.matrix$gene_short_name
fpkm.file = file.path("FPKMmatrix")
write.table(gene.rep.matrix,file = fpkm.file, sep = "  ", row.names = F, col.names = T,quote = F)

sel.rn=rowSums(gene.rep.matrix) != 0
gene.rep.matrix=gene.rep.matrix[sel.rn,]
dim(gene.rep.matrix)
...
cuffgene.table<-cuffgenes@tables
