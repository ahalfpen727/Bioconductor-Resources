...
gnames.entrez = pathview::id2eg(gnames, category = "symbol")
...
if (topN > 0) {
    pv.out.list <- sapply(path.ids2.upExpr[1:topN],
                          function(pid) pathview(gene.data =  exp.fc, pathway.id = pid,  species = species, out.suffix = "cuffdiff", kegg.dir = ".", kegg.native=TRUE))
...
##                       function(pid) pathview(gene.data =  exp.fc, pathway.id = pid,  species = species, out.suffix = "cuffdiff", kegg.dir = outDir, kegg.native=TRUE))
