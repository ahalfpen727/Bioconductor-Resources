...
cols<-c("SYMBOL","PATH", "GO")
chrom <- select(org.Hs.eg.db, keys=keys(eids, keytype="ENTREZID"), columns=c("SYMBOL","PATH","GOG"), keytype="ENTREZID")
...
ensembl_human = useMart("ensembl", dataset="hsapiens_gene_ensembl"); key="entrezgene"
columns <- c("entrezgene","go_id","name_1006","chromosome_name",
             "band","strand","ensembl_gene_id","hgnc_symbol","description")

human.annotation <- getBM(attributes=columns, filters=key, values=eids, mart=ensembl_human)
write.table(human.annotation, "humannet.annotation.baiomart.txt",
            quote = FALSE, sep = "\t", row.names = FALSE)
write.table(human.annotation, "/home/drewjh/umb_triley/Reference-Genomes-and-GeneSets/humannet.annotation.baiomart.txt",quote = FALSE, sep = "\t", row.names = FALSE)
...
write.table(human.annotation, "/home/drewjh/umb_triley/Reference-Genomes-and-GeneSets/humannet.txt",quote = FALSE, sep = "\t", row.names = FALSE)
...
edgelist.url = paste(base.url,
                     "networks?format=edgelist&title=HumanNet&collection=human",
                     sep="/")
