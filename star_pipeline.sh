...
Now, go back to the command (by using the up arrow) and change it so that all the filenames are for the other sample. You can use -Arrow keys to go left and right by entire words to make it easier to get to the parts you need to change:
...
Go to the STAR github page and copy the the URL for the git repository after clicking on the “Clone or download” button. Then use that URL to clone the repo:
